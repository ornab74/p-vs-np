...
| T7g | Every Boolean gate function has a unique minimal orientation equal to its set of decreasing hypercube coordinates | proved | `ORIENTATION_CERTIFICATE_MODEL.md`; finite verification is exponential in gate fan-in but exact. |
| T7h | The formal layered language inherits a depth lower bound against circuits whose every gate has orientation weight at most `w` | proved conditionally on the published Koroth--Sarma CLIQUE theorem and exact projection | `SPARSE_ORIENTATION_TRANSFER.md`; this is a restricted depth result, not an unrestricted size lower bound. |
...
| T9c | Sparse orientation alone separates unrestricted `P/poly` | false / insufficient | The unrestricted assumption supplies circuits with no known per-gate orientation bound. |
...
2. Search for a graph-sensitive semantic identity that bounds canonical orientation defect or per-gate orientation.
...
5. Keep the few-NOT and sparse-orientation theorems as completed restricted endpoints and do not describe them as unrestricted separation.